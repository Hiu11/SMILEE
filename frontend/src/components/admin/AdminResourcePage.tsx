    ...
    <div className="space-y-4 md:space-y-5 lg:space-y-6 pb-6">
      <Reveal direction="scale" className="flex flex-col justify-between gap-4 md:gap-5 sm:flex-row sm:items-center rounded-3xl bg-linear-to-r from-slate-900 to-slate-800 p-5 md:p-6 lg:p-8 shadow-xl shadow-slate-900/10 dark:from-slate-900 dark:to-slate-950 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-black tracking-tight text-white">{title}</h1>
          <p className="mt-2 text-sm font-medium text-slate-300 max-w-xl">{description}</p>
        </div>
        <div className="relative z-10 flex gap-3">
          <Button onClick={load} disabled={loading} variant="outline" className="h-10 md:h-11 lg:h-12 rounded-xl bg-white/10 text-white hover:bg-white/20 border-white/20 backdrop-blur-sm transition-all shadow-sm font-bold px-4 md:px-5">
        ...
        <div className="grid items-start gap-5 xl:grid-cols-[340px_1fr]">
          ...
          <Card className="rounded-3xl border-slate-200/60 shadow-sm backdrop-blur-md dark:border-slate-800/60 dark:bg-slate-900/80 overflow-hidden">
            <CardContent className="p-0">
              <div className="flex flex-col gap-3 md:gap-4 border-b border-slate-100 p-4 md:p-5 lg:p-6 dark:border-slate-800/50 sm:flex-row sm:items-center sm:justify-between">
                <div className="relative max-w-md flex-1">
                  <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" />
                  <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm kiếm..." className="h-10 md:h-11 lg:h-12 rounded-xl bg-slate-50/50 pl-11 text-sm font-medium transition focus:scale-[1.01] focus:ring-2 focus:ring-indigo-500 dark:bg-slate-950/50" />
                </div>

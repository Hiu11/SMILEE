    ...
    <div className="space-y-4 md:space-y-5 lg:space-y-6 pb-6">
      <Reveal direction="scale" className="flex flex-col justify-between gap-4 md:gap-5 sm:flex-row sm:items-center rounded-3xl bg-linear-to-r from-slate-900 to-slate-800 p-5 md:p-6 lg:p-8 shadow-xl shadow-slate-900/10 dark:from-slate-900 dark:to-slate-950 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-black tracking-tight text-white">Quản lý nha sĩ</h1>
        ...
        <div className="relative z-10 flex gap-3">
          <Button onClick={loadDoctors} disabled={loading} variant="outline" className="h-10 md:h-11 lg:h-12 rounded-xl bg-white/10 text-white hover:bg-white/20 border-white/20 backdrop-blur-sm transition-all shadow-sm font-bold px-4 md:px-5">
            <RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />
            Làm mới
          </Button>
          <Button asChild className="h-10 md:h-11 lg:h-12 rounded-xl bg-linear-to-r from-emerald-500 to-teal-500 px-5 md:px-6 text-white shadow-lg shadow-emerald-500/30 hover:shadow-emerald-500/50 hover:-translate-y-0.5 transition-all font-bold">
        ...
        <Card className="border-slate-200/60 shadow-sm backdrop-blur-md dark:border-slate-800/60 dark:bg-slate-900/80 rounded-3xl overflow-hidden">
          <CardContent className="flex flex-col gap-3 md:gap-4 p-4 md:p-5 md:flex-row md:items-center md:justify-between">
            <div className="relative flex-1 md:max-w-md">
              ...
              <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm theo tên, email hoặc số điện thoại..." className="h-10 md:h-11 lg:h-12 rounded-xl bg-slate-50/50 pl-11 text-sm font-medium transition focus:scale-[1.01] focus:ring-2 focus:ring-emerald-500 dark:bg-slate-950/50" />
            </div>
      ...
      <Stagger className="grid grid-cols-1 gap-4 md:gap-6 md:grid-cols-2 xl:grid-cols-3">
          ...
          return (
            <StaggerItem key={doctor.id}>
              <Card className="group relative overflow-hidden rounded-3xl border border-slate-200/60 bg-white/80 shadow-sm backdrop-blur-md transition-all hover:-translate-y-1 hover:shadow-xl dark:border-slate-800/60 dark:bg-slate-900/80 hover:border-emerald-300 dark:hover:border-emerald-800">
                ...
                <CardContent className="relative z-10 -mt-8 p-4 sm:p-6">
                  <div className="flex items-start gap-3 md:gap-4">
                    <div className={`flex h-12 w-12 lg:h-16 lg:w-16 shrink-0 items-center justify-center rounded-2xl bg-linear-to-tr ${color} text-xl lg:text-2xl font-black text-white shadow-lg ring-4 ring-white dark:ring-slate-950`}>
                      {doctor.fullName.split(" ").pop()?.[0] ?? "B"}
                    </div>
                    <div className="min-w-0 flex-1 mt-1 md:mt-2">
                      <h3 className="truncate text-base lg:text-lg font-black text-slate-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">BS. {doctor.fullName}</h3>
                  ...
                  <div className="mt-5 md:mt-6 flex gap-2 md:gap-3">
                    <Button size="sm" className="h-9 md:h-10 flex-1 rounded-xl bg-slate-100 text-slate-700 hover:bg-emerald-50 hover:text-emerald-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 font-bold shadow-sm" onClick={() => setSelected(doctor)}>Xem hồ sơ</Button>
                    <Button asChild size="sm" variant="outline" className="h-9 md:h-10 rounded-xl font-bold shadow-sm">
      ...
      {selected ? (
        <Reveal direction="up" delay={0.2}>
          <div className="relative overflow-hidden rounded-3xl border border-emerald-200/50 bg-linear-to-r from-emerald-50 to-teal-50 p-5 md:p-6 lg:p-8 shadow-lg shadow-emerald-900/5 dark:border-emerald-900/50 dark:from-emerald-950/40 dark:to-teal-900/20">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white">Hồ sơ BS. {selected.fullName}</h2>
